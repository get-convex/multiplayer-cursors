...
import { compressSigned, uncompressSigned } from "./FastIntegerCompression";
import {
  runLengthEncode,
  deltaEncode,
  quantize,
...
export class HistoricalObject<T extends Record<string, number>> {
  ...
  constructor(fields: FieldConfig, data: T) {
    if (fields.length >= MAX_FIELDS) {
      throw new Error(
        `HistoricalObject can have at most ${MAX_FIELDS} fields.`
      );
    }
  ...
  checkShape(data: any) {
    for (const [key, value] of Object.entries(data)) {
      ...
      if (typeof value !== "number") {
        throw new Error(
          `HistoricalObject only supports numeric values, found: ${JSON.stringify(
            value
          )}`
        );
      }
    ...
    for (const [key, value] of Object.entries(data)) {
      ...
      if (currentValue !== value) {
        ...
        if (samples.length > 0) {
          ...
          if (now < last.time) {
            throw new Error(
              `Server time moving backwards: ${now} < ${last.time}`
            );
          }
          if (now === last.time) {
            last.value = value;
...
export function packSampleRecord(
  ...
  for (let fieldNumber = 0; fieldNumber < fields.length; fieldNumber += 1) {
    ...
    const encodedTimestamps = runLengthEncode(
      deltaEncode(timestamps.slice(1), initialTimestamp)
    );
    const compressedTimestamps = compressSigned(encodedTimestamps);
    if (compressedTimestamps.byteLength >= 1 << 16) {
      throw new Error(
        `Compressed buffer too long: ${compressedTimestamps.byteLength}`
      );
    }
    ...
    new Uint8Array(out, pos, compressed.byteLength).set(
...
export function unpackSampleRecord(fields: FieldConfig, buffer: ArrayBuffer) {
  ...
  const expectedConfigHash = xxHash32(
    new Uint8Array(packFieldConfig(normalizedFields))
  );
  ...
  if (configHash !== expectedConfigHash) {
    throw new Error(
      `Config hash mismatch: ${configHash} !== ${expectedConfigHash}`
    );
  }
  ...
  while (pos < buffer.byteLength) {
    ...
    const compressedTimestampBuffer = buffer.slice(
      pos,
    ...
    if (timestamps.length + 1 !== values.length) {
      throw new Error(
        `Invalid sample record: ${timestamps.length} + 1 !== ${values.length}`
      );
    }
